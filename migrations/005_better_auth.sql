-- Better Auth account layer (auth.ts): "user"/session/account/verification.
-- Generated by `npx @better-auth/cli generate` against better-auth 1.7.5,
-- made idempotent (IF NOT EXISTS) per the migrations/ replay discipline.
-- NOTE: Better Auth's quoted "user" table is distinct from our existing
-- `users` table; linking accounts to tournament ownership lands with the
-- tournament-create work.
